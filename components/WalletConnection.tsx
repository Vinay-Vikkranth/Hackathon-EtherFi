...
export function WalletConnection() {
      ...
      <div className="bg-[#2a2a2a] p-4 rounded-lg border border-[#3a3a3a]">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <p className="text-sm text-[#a0a0a0] font-medium">Wallet Connected</p>
              ...
              {balanceLoading && (
                <p className="text-xs text-[#6b6b6b] mt-1">Loading balance...</p>
              )}
            </div>
          </div>
          <button
  ...
  return (
    <div className="bg-[#2a2a2a] p-4 rounded-lg border border-[#3a3a3a]">
      <div className="flex items-center justify-between">
        <div>
          <p className="text-sm text-white font-medium mb-1">Connect your wallet</p>
          <p className="text-xs text-[#a0a0a0]">Connect MetaMask to view your balance</p>
        </div>
        <button
          onClick={async () => {
            try {
      ...
      {hasMetaMask === false && (
        <div className="mt-3 p-3 bg-yellow-500/10 border border-yellow-500/30 rounded-lg">
          <p className="text-xs text-[#f59e0b] font-medium mb-1">⚠️ MetaMask Not Detected</p>
      ...
      {hasMetaMask === true && connectors.length === 0 && (
        <p className="text-xs text-yellow-400 mt-2">
          MetaMask detected but connector not available. Please refresh the page.
        </p>
      )}
    </div>
  );
